...
export class User {
  ...
  @Column({ type: 'varchar', length: 255, nullable: true, name: 'trade_password_hash', comment: '交易密码哈希' })
  tradePasswordHash: string;

  @Column({ type: 'smallint', name: 'has_trade_password', default: 0, comment: '是否设置交易密码: 0否 1是' })
  @Index('idx_trade_password')
  hasTradePassword: number;
  ...
  @Column({ type: 'decimal', precision: 20, scale: 2, name: 'total_recharge', default: '0', comment: '累计充值金额(USDT)' })
  ...
  @Column({ type: 'varchar', length: 20, default: 'user', comment: '用户角色: user=普通用户, moderator=版主, admin=管理员' })
  role: string;
  ...
  @Column({ type: 'varchar', length: 50, nullable: true, name: 'tron_address', comment: '用户TRON充值地址（TRC20）' })
  @Index('idx_tron_address')
  tronAddress: string;

  @Column({ type: 'text', nullable: true, comment: '管理员备注' })
  remark: string;

  @Column({ type: 'smallint', name: 'is_internal', default: 0, comment: '是否内部用户: 0否 1是，内部用户充提不计入总账' })
  isInternal: number;

  @Column({ type: 'bigint', unsigned: true, nullable: true, name: 'assigned_admin_id', comment: '分配的管理员ID' })
  @Index('idx_assigned_admin_id')
  assignedAdminId: number;

  @Column({ type: 'decimal', precision: 20, scale: 2, name: 'experience_points', default: '0', comment: '经验值/积分，1U=1积分' })
  experiencePoints: string;

  @Column({ type: 'smallint', name: 'vip_level', default: 0, comment: 'VIP等级: 0启蒙 1准入 2优选 3资本合伙人 4执行官合伙人 5主权合伙人' })
  vipLevel: number;
