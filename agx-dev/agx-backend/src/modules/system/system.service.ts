...
export class SystemService {
   ...
   * 获取公开配置 (过滤敏感配置)
   */
  async getPublicConfigs(group?: string): Promise<any[]> {
    const sensitiveKeys = ['admin_password', 'jwt_secret', 'api_key', 'db_password'];
    const where = group ? { configGroup: group } : {};
    const configs = await this.configRepo.find({ where });
    return configs.filter(c => !sensitiveKeys.some(k => c.configKey.toLowerCase().includes(k)));
  }
  ...
  async getPlatformInfo(): Promise<any> {
    const keys = ['platform_name', 'customer_service_url', 'website_url', 'logo_url'];
    const configs = await this.configRepo.find();
    const info: Record<string, string> = {};
    configs.forEach(c => {
      if (keys.includes(c.configKey)) {
        info[c.configKey] = c.configValue;
      }
    });
    return {
      platformName: info['platform_name'] || 'AGX',
      customerServiceUrl: info['customer_service_url'] || '',
  ...
  async getAnnouncements(): Promise<any[]> {
    try {
      ...
      const notices = await this.noticeRepo
        ...
        .andWhere('(n.start_time IS NULL OR n.start_time <= :now)', { now })
        .andWhere('(n.end_time IS NULL OR n.end_time >= :now)', { now })
        .orderBy('n.sort', 'ASC')
