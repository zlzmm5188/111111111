...
export class SocialService {
  ...
  // ========== 用户资料 ==========
  async getUserProfile(targetId: number, currentUserId?: number): Promise<any> {
    ...
    if (currentUserId && currentUserId !== targetId) {
      const follow = await this.followRepo.findOne({
        where: { userId: currentUserId, followUserId: targetId },
      });
      isFollowing = !!follow;
    }
  ...
  async adminMuteUser(userId: number, until: Date | null): Promise<any> {
    if (until) {
      // 禁言
      await this.userRepo.update({ id: userId }, { socialStatus: 0, muteUntil: until });
    } else {
      // 解除禁言
      await this.userRepo.update({ id: userId }, { socialStatus: 1, muteUntil: null });
    }
    return { success: true };
  }
   ...
   * 获取关注列表
   */
  async getFollowing(userId: number, page = 1, limit = 20): Promise<any> {
    const [list, total] = await this.followRepo.findAndCount({
      where: { userId },
      order: { createdAt: 'DESC' },
      skip: (page - 1) * limit,
      take: limit,
    ...
    const followIds = list.map(f => f.followUserId);
    const users = followIds.length
      ? await this.userRepo.findByIds(followIds)
      : [];
   ...
   * 获取粉丝列表
   */
  async getFollowers(userId: number, page = 1, limit = 20): Promise<any> {
    const [list, total] = await this.followRepo.findAndCount({
    ...
    const followerIds = list.map(f => f.userId);
    const users = followerIds.length
      ? await this.userRepo.findByIds(followerIds)
      : [];
    ...
    const mutualCheck = await Promise.all(
      followerIds.map(async (fId) => {
        const mutual = await this.followRepo.findOne({
          where: { userId, followUserId: fId },
        });
        return { userId: fId, isMutual: !!mutual };
      })
    );
    const mutualMap = new Map(mutualCheck.map(m => [m.userId, m.isMutual]));
    ...
    return {
      list: users.map(u => ({
        ...
        bio: u.bio,
        isVerified: u.isVerified,
        isMutual: mutualMap.get(u.id) || false,
      })),
      total,
      page,
      limit,
   ...
   * 获取好友请求列表
   */
  async getFriendRequests(userId: number, page = 1, limit = 20): Promise<any> {
    const [list, total] = await this.friendRequestRepo.findAndCount({
      where: { toUserId: userId, status: 0 },
      order: { createdAt: 'DESC' },
      skip: (page - 1) * limit,
      take: limit,
    });

    const fromUserIds = list.map(r => r.fromUserId);
    const users = fromUserIds.length
      ? await this.userRepo.findByIds(fromUserIds)
      : [];
    const userMap = new Map(users.map(u => [u.id, u]));
   ...
   * 标记消息已读
   */
  async markMessagesRead(userId: number, conversationId: number): Promise<any> {
    const conversation = await this.conversationRepo.findOne({
      where: { id: conversationId },
    });
    if (!conversation) throw new BadRequestException('会话不存在');
  ...
  async getUnreadCount(userId: number): Promise<any> {
    const conversations = await this.conversationRepo.find({
      where: [
        { userAId: userId, status: 1 },
        { userBId: userId, status: 1 },
      ],
    });
    ...
    for (const c of conversations) {
      if (c.userAId === userId) {
        totalUnread += c.unreadA || 0;
      } else {
        totalUnread += c.unreadB || 0;
      }
    }
