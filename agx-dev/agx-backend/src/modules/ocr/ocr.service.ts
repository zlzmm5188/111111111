  ...
  async detectLiveness(imageSource: string | Buffer): Promise<LivenessResult> {
    ...
    try {
      ...
      if (typeof imageSource === 'string') {
        ...
        const imageBuffer = fs.readFileSync(imageSource);
        base64Image = imageBuffer.toString('base64');
        mimeType = this.getMimeType(imageSource);
      } else {
        base64Image = imageSource.toString('base64');
      }

      const result = await this.callDeepseekVision(base64Image, mimeType, 'liveness');
      ...
      this.logger.error(`真人检测错误: ${error.message}`);
      return { isReal: false, confidence: 0, reason: error.message };
    }
  }
