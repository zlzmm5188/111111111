...
export class PoolService {
  ...
  async subscribe(userId: number, dto: SubscribePoolDto) {
    ...
    try {
      ...
      return {
        ...
        incomeCoinSymbol: incomeCoin?.symbol || coinSymbol,
        // 收益信息
        dailyIncome: dailyIncome,
        dailyIncomeDisplay: `${dailyIncome} ${incomeCoin?.symbol || coinSymbol}`,
        annualIncome: annualIncome,
        annualIncomeDisplay: `${annualIncome} ${incomeCoin?.symbol || coinSymbol}`,
      ...
      for (const holding of holdings) {
        try {
          ...
          try {
            ...
            // 处理邀请返佣（异步，不影响主流程）
            this.processPoolCommission(holding.userId, holding.id, dailyIncomeStr,
              (await this.coinRepo.findOne({ where: { id: incomeCoinId } }))?.symbol || 'USDT'
            ).catch(err => {
  ...
  private async processMaturedHolding(queryRunner: any, holding: any, dailyIncomeStr: string, incomeCoinId: number) {
    ...
    // 3. 处理分红（如果启用）
    if (holding.product.enableDividend === 1) {
      const dividendAmount = new Decimal(holding.amount)
        .mul(holding.product.dailyRate)
        .mul(holding.product.lockDays || 1)
        .mul(0.1) // 分红比例为总收益的10%
        .toFixed(8);

      const dividendBalance = new Decimal(incomeWallet.balance)
        .plus(dividendAmount)
        .toString();
   ...
   */
  private async processPoolCommission(userId: number, holdingId: number, incomeAmount: string, coin: string) {
    try {
      // 获取用户的直接邀请人
      ...
      // 根据有效好友数量获取返佣比例
      const commissionRate = await this.getCommissionRateByInviteCount(validInviteCount);
      ...
      if (!coinEntity) {
        this.logger.error(`未找到币种: ${coin}`);
        return;
      }

      // 获取邀请人的钱包
      const inviterWallet = await this.getOrCreateWallet(inviterId, coinEntity.id);
      ...
      try {
        ...
        this.logger.log(`矿池返佣成功: 邀请人${inviterId} (有效好友${validInviteCount}人, ${(commissionRate*100).toFixed(1)}%) 从用户${userId}获得返佣${commissionAmount} ${coin}`);
      } catch (error) {
        await queryRunner.rollbackTransaction();
        throw error;
      } finally {
  ...
  /**
   * 检查用户是否为有效用户（已建仓）
   * 有效好友建仓判定标准：常规产品、周期≥7天、金额≥100
   */
  private async checkIsValidUser(userId: number): Promise<boolean> {
    try {
      const validHoldings = await this.holdingRepo
        .createQueryBuilder('h')
        .innerJoinAndSelect('h.product', 'p')
        .where('h.userId = :userId', { userId })
        .andWhere('h.status = :status', { status: 1 })
        .andWhere('p.lockDays >= :minDays', { minDays: 7 })
        .andWhere('p.type != :experienceType', { experienceType: 'experience' })
        .getMany();

      return validHoldings.some(holding => {
  ...
  private async getValidInviteCount(inviterId: number): Promise<number> {
    const directInvites = await this.userInviteRepo.find({
      where: { inviterId, level: 1 }
    });

    if (directInvites.length === 0) return 0;

    let validCount = 0;
    for (const invite of directInvites) {
      const isValid = await this.checkIsValidUser(invite.userId);
