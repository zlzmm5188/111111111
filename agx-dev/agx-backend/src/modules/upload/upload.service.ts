...
export class UploadService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.bucketName = this.configService.get<string>('CF_R2_BUCKET_NAME', 'agx-uploads');
    this.publicUrl = this.configService.get<string>('CF_R2_PUBLIC_URL', '');
    
    // 检查是否配置了 Cloudflare R2
    this.useCloudflare = !!(accountId && accessKeyId && secretAccessKey);
    ...
    if (this.useCloudflare) {
      ...
      this.logger.log('Cloudflare R2 存储已启用');
    } else {
      this.logger.warn('Cloudflare R2 未配置，将使用本地存储（不推荐用于生产环境）');
    }
  }
