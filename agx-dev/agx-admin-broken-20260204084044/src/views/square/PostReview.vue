    ...
    >
      <a-form :model="batchRejectForm">
        ...
        <a-alert type="info" :show-icon="false">
          将批量拒绝选中的 {{ selectedIds.length }} 个帖子
        </a-alert>
      </a-form>
    </a-modal>
  </div>
...
const batchApprove = async () => {
  if (selectedIds.value.length === 0) {
    Message.warning('请选择要通过的帖子');
    return;
  }

  if (!confirm(`确定要批量通过选中的 ${selectedIds.value.length} 个帖子吗？`)) {
    return;
  }

  try {
    const response = await api.square.batchReviewPosts({
...
const confirmBatchReject = async () => {
  if (!batchRejectForm.reason.trim()) {
    Message.error('请输入拒绝原因');
    return;
  }
