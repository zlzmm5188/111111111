...
const fetchData = async () => {
  ...
  try {
    ...
    if (res.code === 0 && res.data) {
      const toggleMap = {}
      res.data.forEach(t => { toggleMap[t.key] = t.enabled === 1 })
      
      // 更新各组开关状态
      Object.values(featureGroups).forEach(group => {
        group.items.forEach(item => {
          if (toggleMap[item.key] !== undefined) {
            item.enabled = toggleMap[item.key]
          }
        })
      })
    }
...
const handleSave = async () => {
  ...
  try {
    ...
    Object.values(featureGroups).forEach(group => {
      group.items.forEach(item => {
        toggles.push({ key: item.key, enabled: item.enabled ? 1 : 0 })
      })
    })
