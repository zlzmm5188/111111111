...
const fetchStats = async () => {
  try {
    ...
    if (res.code === 0 && res.data) {
      ...
      stats.todayUsers = d.todayUsers || 0
      stats.weekUsers = d.weekUsers || 0
      stats.totalRecharge = toNum(d.totalRecharge)
      stats.todayRecharge = toNum(d.todayRecharge)
      stats.totalWithdraw = toNum(d.totalWithdraw)
      stats.pendingWithdraw = d.pendingWithdraw || 0
      stats.totalOrders = d.totalOrders || 0
      stats.todayOrders = d.todayOrders || 0
      stats.totalAssets = toNum(d.totalAssets)
      stats.poolTotal = toNum(d.totalPool)
      stats.pendingKyc = d.pendingKyc || 0
    }
  } catch (e) {
    console.error(e)
  }
...
/* 紧凑布局优化 */
.stat-card.blue { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: #fff; }
.stat-card.blue :deep(.arco-stat-title),
.stat-card.blue :deep(.arco-stat-value) { color: #fff !important; }
.stat-card.green { background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); color: #fff; }
.stat-card.green :deep(.arco-stat-title),
.stat-card.green :deep(.arco-stat-value) { color: #fff !important; }
.stat-card.orange { background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: #fff; }
.stat-card.orange :deep(.arco-stat-title),
.stat-card.orange :deep(.arco-stat-value) { color: #fff !important; }
.stat-card.purple { background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); color: #fff; }
.stat-card.purple :deep(.arco-stat-title),
.stat-card.purple :deep(.arco-stat-value) { color: #fff !important; }
.stat-card.blue .stat-footer,
.stat-card.green .stat-footer,
.stat-card.orange .stat-footer,
.stat-card.purple .stat-footer { color: rgba(255,255,255,0.85); }
