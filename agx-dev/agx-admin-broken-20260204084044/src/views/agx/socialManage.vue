...
const handleSearchFriends = async () => {
  ...
  try {
    ...
    if (res.code === 0 && res.data) {
      friendData.value = res.data.list || []
    }
  } catch (error) {
...
const handleSearchMessages = async () => {
  ...
  try {
    const params = {
      senderId: searchForm.senderId || undefined,
      receiverId: searchForm.receiverId || undefined
    }
    if (searchForm.dateRange && searchForm.dateRange.length === 2) {
      params.startDate = searchForm.dateRange[0]
      params.endDate = searchForm.dateRange[1]
    }
    const res = await api.getAllMessages(params)
    if (res.code === 0 && res.data) {
      messageData.value = res.data.list || []
    }
  } catch (error) {
    console.error('获取私聊记录失败:', error)
    Message.error('获取私聊记录失败')
  } finally {
    loading.value = false
  }
}
...
const handleSearchUsers = async () => {
  ...
  try {
    const res = await api.getUserList({
      keyword: searchForm.userKeyword || undefined,
      socialStatus: searchForm.socialStatus || undefined
    })
    if (res.code === 0 && res.data) {
      userData.value = (res.data.list || []).map(u => ({
        ...u,
        canBeFriended: u.canBeFriended ?? 1,
        canChat: u.canChat ?? 1,
        socialStatus: u.socialStatus ?? 1,
...
const handleRemoveFriend = async (record) => {
  try {
    const res = await request({
      url: '/api/admin/social/remove-friend',
      method: 'post',
      data: { userAId: record.userId, userBId: record.friendId }
    })
    if (res.code === 0) {
      Message.success(`已强制解除 ${record.userAName} 和 ${record.userBName} 的好友关系`)
      handleSearchFriends()
    } else {
      Message.error(res.msg || '解除好友关系失败')
    }
  } catch (error) {
...
const handleMuteSubmit = async () => {
  try {
    const res = await request({
      url: `/api/admin/social/user/${muteForm.userId}/mute`,
      method: 'post',
      data: { duration: muteForm.duration, reason: muteForm.reason }
    })
    if (res.code === 0) {
...
const handleUnmuteUser = async (record) => {
  try {
    const res = await request({
      url: `/api/admin/social/user/${record.id}/mute`,
      method: 'post',
      data: { duration: 0 }
    })
    if (res.code === 0) {
...
const handleBanSocial = async (record) => {
  try {
    const res = await request({
      url: `/api/admin/social/user/${record.id}/social-status`,
      method: 'post',
      data: { status: -1 }
    })
    if (res.code === 0) {
      ...
      Message.error(res.msg || '社交封禁失败')
    }
  } catch (error) {
    console.error('社交封禁失败:', error)
    Message.error('社交封禁失败')
  }
}

const handleCanBeFriendedChange = async (record, val) => {
  try {
    const res = await request({
      url: `/api/admin/social/user/${record.id}/can-be-friended`,
      method: 'post',
      data: { canBeFriended: val }
    })
    if (res.code === 0) {
      Message.success(val ? '已允许被加好友' : '已禁止被加好友')
    } else {
      record.canBeFriended = val ? 0 : 1
...
const handleCanChatChange = async (record, val) => {
  try {
    const res = await request({
      url: `/api/admin/social/user/${record.id}/can-chat`,
      method: 'post',
      data: { canChat: val }
    })
    if (res.code === 0) {
