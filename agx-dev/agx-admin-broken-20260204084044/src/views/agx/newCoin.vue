<template>
  <div class="agx-page">
    ...
    <div class="agx-card agx-table">
      ...
      >
        <template #columns>
          <a-table-column title="发售信息" :width="220">
          ...
          <a-table-column title="发售价格" :width="120" align="center">
            <template #cell="{ record }">
              <span class="price-value">${{ record.issuePrice }}</span>
            </template>
          </a-table-column>
          <a-table-column title="发售额度" :width="180">
            <template #cell="{ record }">
    ...
    >
      <a-form :model="formData" layout="vertical" class="agx-form">
        ...
        <div class="form-section">
          <div class="section-title">发售设置</div>
          <a-row :gutter="16">
            <a-col :span="8">
              <a-form-item label="发售价格(USDT)" required>
                <a-input-number 
            ...
            <a-col :span="8">
              <a-form-item label="发售总量" required>
                <a-input-number 
        ...
        <div class="form-section">
          <div class="section-title">返利设置</div>
          <a-row :gutter="16">
            <a-col :span="6">
              <a-form-item label="启用返利">
                <a-switch v-model="formData.enableCommission" :checked-value="1" :unchecked-value="0" />
              </a-form-item>
            </a-col>
            <a-col :span="6">
              <a-form-item label="一级返利(%)">
                <a-input-number 
                  v-model="formData.commissionRate1" 
                  :min="0" 
                  :max="100"
                  :precision="2"
                  :disabled="!formData.enableCommission"
                  style="width: 100%" 
                  placeholder="5"
                />
              </a-form-item>
            ...
            <a-col :span="6">
              <a-form-item label="三级返利(%)">
                <a-input-number 
                  v-model="formData.commissionRate3" 
                  :min="0" 
                  :max="100"
                  :precision="2"
                  :disabled="!formData.enableCommission"
...
const editCoin = (record) => {
  ...
  Object.assign(formData, {
    ...
    enableCommission: record.enableCommission || 0,
    commissionRate1: (record.commissionRate1 || 0.05) * 100,
    commissionRate2: (record.commissionRate2 || 0.03) * 100,
    commissionRate3: (record.commissionRate3 || 0.02) * 100
  })
  modalVisible.value = true
}
...
const handleSubmit = async () => {
  ...
  try {
    const submitData = {
      ...
      enableCommission: formData.enableCommission,
      commissionRate1: formData.commissionRate1 / 100,
      commissionRate2: formData.commissionRate2 / 100,
      commissionRate3: formData.commissionRate3 / 100
    }
