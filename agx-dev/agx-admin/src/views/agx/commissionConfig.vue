...
const fetchConfig = async () => {
  ...
  try {
    ...
    if (res.code === 0 && res.data) {
      ...
      // 返佣配置
      const sources = getValue('commission_sources', 'contract,gold')
      commissionConfig.sources = sources.split(',')
      commissionConfig.level1 = Number(getValue('commission_level1', 20))
      commissionConfig.level2 = Number(getValue('commission_level2', 10))
      commissionConfig.maxLevel = Number(getValue('commission_max_level', 2))
      commissionConfig.settleCycle = getValue('commission_settle_cycle', 'realtime')
      
      // 手续费配置
      shareConfig.contractFee = Number(getValue('fee_contract', 0.5))
      shareConfig.spotFee = Number(getValue('fee_spot', 0.1))
      shareConfig.goldFee = Number(getValue('fee_gold', 0.1))
      shareConfig.withdrawFee = Number(getValue('fee_withdraw', 5))
      
      // 特殊规则
      specialConfig.newUserBonus = Number(getValue('bonus_new_user', 5))
      specialConfig.firstDepositBonus = Number(getValue('bonus_first_deposit', 5))
      specialConfig.minCommission = Number(getValue('commission_min', 0.01))
    }
  } catch (error) {
...
const handleSave = async () => {
  ...
  try {
    const configs = [
      { key: 'commission_sources', value: commissionConfig.sources.join(',') },
      { key: 'commission_level1', value: String(commissionConfig.level1) },
      { key: 'commission_level2', value: String(commissionConfig.level2) },
      { key: 'commission_max_level', value: String(commissionConfig.maxLevel) },
      { key: 'commission_settle_cycle', value: commissionConfig.settleCycle },
      { key: 'fee_contract', value: String(shareConfig.contractFee) },
      { key: 'fee_spot', value: String(shareConfig.spotFee) },
      { key: 'fee_gold', value: String(shareConfig.goldFee) },
      { key: 'fee_withdraw', value: String(shareConfig.withdrawFee) },
      { key: 'bonus_new_user', value: String(specialConfig.newUserBonus) },
      { key: 'bonus_first_deposit', value: String(specialConfig.firstDepositBonus) },
      { key: 'commission_min', value: String(specialConfig.minCommission) }
    ]
