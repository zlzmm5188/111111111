...
const whiteRoute = ['login', 'splash', 'mineDoc', 'interfaceList', 'interfaceCode', 'signature']

// 检查是否已完成启动页验证
const hasSplashVerified = () => {
  return sessionStorage.getItem('agx_splash_verified') === 'true'
}
...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.name !== 'splash' && !hasSplashVerified()) {
    // 如果 URL 中有 verified=true 参数，标记为已验证
