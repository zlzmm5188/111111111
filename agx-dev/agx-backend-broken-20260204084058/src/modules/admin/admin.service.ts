...
export class AdminService {
  ...
  async deleteAdmin(id: number, currentAdminId?: number) {
    ...
    if (currentAdminId && id === currentAdminId) {
      throw new BusinessException(1002, '不能删除自己的账号');
    }
  ...
  async getUserList(dto: UserListDto, currentAdminGroup?: number, currentAdminId?: number) {
    // ✅ 记录管理员操作日志
    if (currentAdminId) {
      await this.logAdminAction(currentAdminId, 'view', 'user_list');
    }
    const page = this.safeInt(dto.page, 1);
    const pageSize = this.safeInt(dto.pageSize, 20);
    const status = this.safeOptionalInt(dto.status);
    ...
    const query = this.userRepo.createQueryBuilder('user')
      .select(['user.id', 'user.username', 'user.email', 'user.status', 'user.createdAt']) // 只选择必要字段
  ...
  async settleContractOrder(id: number, dto: any, adminId?: number, ip?: string) {
    ...
    if (result === 1) {
      ...
      // 给用户加钱
      const wallet = await this.walletRepo.findOne({ where: { userId: order.userId, coinId: COIN_IDS.AGX } });
      if (wallet) {
  ...
  async manualRecharge(dto: any, adminId?: number, ip?: string) {
    ...
    const recharge = this.rechargeRepo.create({
      ...
      orderNo,
      coin,
      chain: 'TRC20',  // 统一使用TRC20
      amount,
      txHash: 'ADMIN_' + orderNo,  // 管理员手动充值标识
      fromAddress: '管理员充值',
  ...
  async reviewWithdraw(id: number, dto: any, adminId?: number, ip?: string) {
    ...
    // 状态检查：
    // - 审核通过/拒绝：只能从状态0（待审核）修改
    // - 确认打款：只能从状态1（待打款）修改为状态3（已完成）
    if (dto.status === 1 || dto.status === 2) {
      // 审核通过或拒绝
      if (withdraw.status !== 0) {
        throw new BusinessException(6002, '该提现订单已审核');
      }
    } else if (dto.status === 3) {
      // 确认打款
      if (withdraw.status !== 1) {
        throw new BusinessException(6003, '只有已审核通过的订单才能确认打款');
      }
    }
    ...
    if (dto.status === 3) {
      ...
      if (coinEntity) {
        ...
        if (wallet) {
          ...
          // 扫除冻结金额
          wallet.frozen = Math.max(0, frozenAmount - withdrawAmount).toFixed(8);
          await this.walletRepo.save(wallet);
          ...
          const log = this.assetLogRepo.create({
            ...
            balanceAfter: wallet.balance,  // 余额不变，只扫冻结
            refNo: withdraw.orderNo,
            remark: '提现完成',
          });
          await this.assetLogRepo.save(log);
  ...
  async ensureDefaultAdmin() {
    ...
    if (!admin) {
      const defaultPassword = process.env.DEFAULT_ADMIN_PASSWORD || 'Admin@2025!';
      ...
      this.logger.log('Default admin created, please check env for password');
    }
  }
  ...
  async logAdminAction(
    ...
    try {
      ...
      this.logger.error('记录操作日志失败:', e.message);
    }
  }
