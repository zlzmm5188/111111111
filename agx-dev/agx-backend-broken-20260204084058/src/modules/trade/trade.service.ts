...
import { TradingPair, SpotOrder, Wallet, Coin, CoinIssue, CoinSubscription, AssetLog, Config, User } from '../../entities';
...
export class TradeService {
  ...
  async subscribeCoin(userId: number, dto: SubscribeCoinDto) {
    ...
    if (!user.tradePasswordHash) {
      throw new BusinessException(5022, '请先设置交易密码');
    }

    const isPasswordValid = await bcrypt.compare(dto.tradePassword, user.tradePasswordHash);
    if (!isPasswordValid) {
      throw new BusinessException(5027, '交易密码错误');
    }
    ...
    // 检查后台配置的倒计时设置
    const countdownEnabled = await this.configRepo.findOne({ where: { configKey: 'countdown_enabled' } });
    const countdownEndTime = await this.configRepo.findOne({ where: { configKey: 'countdown_end_time' } });
    
    // 如果倒计时开启且未结束，则不允许购买
    if (countdownEnabled?.configValue === '1' && countdownEndTime?.configValue) {
      const endTime = new Date(countdownEndTime.configValue);
      if (now < endTime) {
        throw new BusinessException(5021, '认购倒计时未结束，请稍候');
      }
    }
    ...
    try {
      ...
      const subscription = this.subscriptionRepo.create({
        ...
        winAmount: dto.buyAmount, // 全额中签
        refundAmount: 0,
        status: IEO_STATUS.WON, // 已中签待发币
      });
      ...
      const assetLog = this.assetLogRepo.create({
        ...
        remark: `IEO新币认购: ${issue.coinSymbol}`,
      });
      ...
      return {
        ...
        data: {
          ...
          winAmount: subscription.winAmount,
