...
export class GoldService {
  ...
  /**
   * 获取持币生金状态
   * 简化版：无等级系统，只有100 AGX激活门槛和统一收益率
   */
  async getHoldingStatus(userId: number, agxBalance: string) {
    const balance = new Decimal(agxBalance);
    
    // 从配置表获取第一条有效配置（统一收益率）
    const config = await this.holdingConfigRepository.findOne({
      where: { status: 1 },
      order: { sortOrder: 'ASC' },
    });
    
    // 默认配置：100 AGX 激活，0.00005 oz/AGX/天
    const minAgxRequired = config?.minAgx || '100';
    const dailyOzRate = config?.dailyOzRate || '0.00005';
    
    // 判断是否达到激活门槛
    const isQualified = balance.gte(minAgxRequired);
    ...
    // 计算距激活还差多少
    const toActivate = isQualified 
      ? 0 
      : Math.max(0, parseFloat(minAgxRequired) - parseFloat(agxBalance));
    ...
    return {
      ...
      toActivate,
      dailyRate: parseFloat(dailyOzRate) * 100, // 转为百分比显示 (如 0.005%)
      dailyOzRate,
      estimatedDailyOz,
      todayMinBalance: snapshot?.minBalance || agxBalance,
      status: isQualified ? '生金中' : '未激活',
    };
  }
  ...
  async subscribeFinance(userId: number, dto: { productId: number; amount: string }) {
    ...
    // 获取用户USDT钱包
    const wallet = await this.walletRepository.findOne({
      where: { userId, coinId: COIN_IDS.USDT },
    });
