...
export class MemberLevelService {
  ...
  async getUserLevelInfo(userId: number): Promise<LevelInfo> {
    ...
    const totalExperience = parseFloat(user.experiencePoints || '0');
  ...
  async recalculateAllUserLevels(): Promise<{ updated: number }> {
    const users = await this.userRepo.find({
      select: ['id', 'level', 'experiencePoints'],
    });

    let updated = 0;
    for (const user of users) {
      const newLevel = await this.calculateLevel(user.experiencePoints || '0');
