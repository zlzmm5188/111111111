...
- 必要时提供代码示例`,
  ...
  /**
   * 从身份证号码判断性别
   * 中国身份证号码第17位：奇数为男，偶数为女
   */
  private getGenderFromIdNumber(idNumber: string): '男' | '女' | null {
    ...
    if (idNumber.length === 15) {
      const genderCode = parseInt(idNumber.charAt(14), 10);
      return genderCode % 2 === 1 ? '男' : '女';
    }
    return null;
  }
   ...
   * 获取姓氏（取姓名的第一个字）
   */
  private getSurname(realName: string): string {
    if (!realName) return '';
    // 复姓处理
    const compoundSurnames = ['欧阳', '上官', '司马', '东方', '诸葛', '皇甫', '令狐', '慕容', '公孙', '长孙', '宇文', '司徒', '轩辕', '独孤', '南宫', '尉迟'];
    for (const cs of compoundSurnames) {
      if (realName.startsWith(cs)) return cs;
    }
  ...
  private async getUserContext(userId: number): Promise<string> {
    try {
      ...
      if (kyc && kyc.realName) {
        ...
        surname = this.getSurname(realName);
        gender = this.getGenderFromIdNumber(kyc.idNumber);
        honorific = gender === '男' ? `${surname}先生` : gender === '女' ? `${surname}女士` : realName;
      }
...
【重要提示】
请在每次回复开头使用"尊称"来称呼用户，例如："尊敬的${honorific || '用户'}，..."
`;
    } catch (error) {
