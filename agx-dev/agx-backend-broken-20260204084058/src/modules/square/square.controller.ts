import { Controller, Get, Post, Put, Delete, Body, Query, Param, UseGuards, Request, Logger } from '@nestjs/common';
...
export class SquareController {
  ...
  async getFlash(@Query('limit') limit: number = 20) {
    try {
      ...
      this.logger.debug('Finance-news快讯获取失败，使用金十数据');
    }
  ...
  async getNews(
    ...
    try {
      ...
      this.logger.debug('Finance-news新闻获取失败，使用备用源');
    }
  ...
  private async fetchFinanceNews(page: number, pageSize: number) {
    ...
    try {
      ...
      this.logger.debug('金十数据API请求失败，使用备用数据');
    }
  ...
  async getHedraVideos(@Query('type') type: string = 'video') {
    ...
    try {
      ...
      this.logger.error('Hedra API error:', error.message);
    }
