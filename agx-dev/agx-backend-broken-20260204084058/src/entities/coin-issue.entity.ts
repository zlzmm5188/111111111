...
export class CoinIssue {
  ...
  @Column({ length: 500, nullable: true, comment: '币种Logo URL' })
  logo: string;
  ...
  @Column({ name: 'enable_commission', type: 'smallint', default: 0, comment: '是否启用返利: 0否 1是' })
  enableCommission: number;

  @Column({ name: 'commission_rate_1', type: 'decimal', precision: 5, scale: 4, default: 0.05, comment: '一级返利比例' })
  commissionRate1: number;

  @Column({ name: 'commission_rate_2', type: 'decimal', precision: 5, scale: 4, default: 0.03, comment: '二级返利比例' })
  commissionRate2: number;

  @Column({ name: 'commission_rate_3', type: 'decimal', precision: 5, scale: 4, default: 0.02, comment: '三级返利比例' })
